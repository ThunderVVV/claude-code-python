...
DEFAULT_INSTRUCTION_FILES = [
    "AGENTS.md",
    "CLAUDE.md",
    "CONTEXT.md",  # deprecated, kept for compatibility
]

# Environment variable names
ENV_CONFIG_DIR = "OPENCODE_CONFIG_DIR"
ENV_DISABLE_CLAUDE_PROMPT = "OPENCODE_DISABLE_CLAUDE_CODE_PROMPT"
...
class InstructionConfig:
    ...
    def from_env(cls) -> "InstructionConfig":
        ...
        disable_claude_prompt = os.environ.get(ENV_DISABLE_CLAUDE_PROMPT, "").lower() in (
            "1",
            "true",
            "yes",
        )
        disable_project_config = os.environ.get(ENV_DISABLE_PROJECT_CONFIG, "").lower() in (
            "1",
            "true",
            "yes",
        )
...
class InstructionService:
    ...
    def _find_upward(self, filename: str, start_dir: str, stop_dir: Optional[str] = None) -> Optional[str]:
    ...
    def _find_all_upward(self, filename: str, start_dir: str, stop_dir: Optional[str] = None) -> List[str]:
    ...
    async def load_instructions(
        ...
        if paths:
            ...
            for filepath, content in results:
                if content:
                    instructions.append(LoadedInstruction(path=filepath, content=content))
    ...
    async def resolve_nearby_instructions(
        ...
        while current.startswith(root) and current != root:
            ...
            if found:
                ...
                if (found_abs != target and 
                    found_abs not in system_paths and 
                    found_abs not in already_loaded and
                    found_abs not in claims):
                    ...
                    if content:
                        results.append(LoadedInstruction(path=found_abs, content=content))
...
def get_instruction_service(config: Optional[InstructionConfig] = None) -> InstructionService:
