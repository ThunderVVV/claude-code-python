"""Diff widget adapted from Toad's custom Textual diff view."""
...
from textual import events, highlight
...
def loop_last[T](values: Iterable[T]) -> Iterable[tuple[bool, T]]:
    """Iterate values and flag the last item."""
...
class DiffScrollContainer(containers.HorizontalGroup):
    """Horizontally scrollable diff pane that can link to a sibling pane."""
...
class LineContent(Visual):
    """Visual that renders diff lines with per-line background styles."""
...
class LineAnnotations(Widget):
    """A vertical strip with line numbers or diff markers."""
...
def fill_lists[T](list_a: list[T], list_b: list[T], fill_value: T) -> None:
    """Make two lists the same length by extending the shorter one."""
    a_length = len(list_a)
    b_length = len(list_b)
...
class DiffView(containers.VerticalGroup):
    ...
    NUMBER_STYLES = {
        "+": "$foreground on $success 20%",
        "-": "$foreground on $error 20%",
    ...
    EDGE_STYLES = {
        "+": "$foreground 60% on $success 20%",
        "-": "$foreground 60% on $error 20%",
    ...
    async def prepare(self) -> None:
        """Precompute expensive diff structures off the UI thread."""
    ...
    def _highlight_diff_lines(
        ...
        """Highlight character-level changes inside a replaced line block."""
    ...
    def highlighted_code_lines(self) -> tuple[list[Content], list[Content]]:
        """Return syntax-highlighted lines for before and after code."""
    ...
    def get_title(self) -> Content:
        """Return the diff title line."""
        additions, removals = self.counts
        return Content.from_markup(
            "Diff [dim]$path[/dim] ([b]+$additions[/b], [b]-$removals[/b])",
            path=self.path2,
            additions=additions,
            removals=removals,
        ).stylize_before("$foreground")
    ...
    def compose_unified(self) -> ComposeResult:
        ...
        for last, group in loop_last(self.grouped_opcodes):
            ...
            with containers.HorizontalGroup(classes="diff-group"):
                ...
                code_line_styles = [
                    self.LINE_STYLES[annotation] for annotation in annotations
                ]
