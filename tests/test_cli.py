...
def test_resolve_log_path_returns_explicit_log_file() -> None:
    assert resolve_log_path("custom.log", debug=True, tui=True) == "custom.log"


def test_resolve_log_path_is_none_without_debug_or_tui() -> None:
    assert resolve_log_path(None, debug=False, tui=False) is None
