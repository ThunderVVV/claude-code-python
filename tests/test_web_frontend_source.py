...
def test_web_frontend_contains_auto_follow_hooks():
    ...
    assert "if (messagesContainer.value && (force || autoFollowOutput.value))" in WEB_FRONTEND_SOURCE
...
def test_web_frontend_contains_user_context_and_web_indicator_hooks():
    ...
    assert "const currentInputHasWeb = computed(() => hasWebReference(inputText.value));" in WEB_FRONTEND_SOURCE
...
def test_web_frontend_contains_keyboard_parity_hooks():
    ...
    assert "document.addEventListener('keydown', handleGlobalKeydown);" in WEB_FRONTEND_SOURCE
    assert "navigator.clipboard?.writeText" in WEB_FRONTEND_SOURCE
    assert "if (inputHistory.value.length > 1000) inputHistory.value.shift();" in WEB_FRONTEND_SOURCE
